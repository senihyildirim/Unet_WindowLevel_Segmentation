...
val_transform_general = Compose([
    LoadImaged(keys=["image", "zmap", "label"]),
    PreprocessDatad(keys=["image", "zmap", "label"], divider=32),
    ToTensord(keys=["image", "zmap", "label"])
])

def apply_val_transforms(val_data, window_level, window_width):
    transform = WindowLevelTransformd(keys=["image"], window_level=window_level, window_width=window_width)
    val_data = transform(val_data)

    normalize_transform = NormalizeIntensityd(keys=["image", "zmap"], nonzero=True)
    val_data = normalize_transform(val_data)
    ...
    with torch.no_grad():
        for val_data in val_loader:
            ...
            for i in range(len(model_paths)):
                ...
                print(f"Validation with model {model_paths[i]} and window level {window_level} and window width {window_width}")

                val_data_specific = apply_val_transforms(val_data, window_level, window_width)

                transformed_image = val_data_specific["image"].to(device)
            ...
            val_labels_decollated = decollate_batch(val_labels.unsqueeze(0))
...
val_dataset = Dataset(val_json_object['validation'], transform=val_transform_general)
